...
class Ads: UIView, GADBannerViewDelegate, ADBannerViewDelegate {
    let AdUnitID = "ca-app-pub-9546989319201990/2566533461"
    ...
    func initAdMobBanner() {
        if UIDevice.currentDevice().userInterfaceIdiom == UIUserInterfaceIdiom.Pad {
            // iPad banner
            adMobBannerView.adSize =  GADAdSizeFromCGSize(CGSizeMake(self.vc.view.frame.size.width, 90))
            adMobBannerView.frame = CGRectMake(0, self.vc.view.frame.size.height, self.vc.view.frame.size.width, 90)
            
        } else {
            // iPhone banner
            adMobBannerView.adSize =  GADAdSizeFromCGSize(CGSizeMake(self.vc.view.frame.size.width, 50))
            adMobBannerView.frame = CGRectMake(0, self.vc.view.frame.size.height, self.vc.view.frame.size.width, 50)
        }
